...
import { UploadCloud, CheckSquare, MapPin } from "lucide-react";
...
export default function AddRoom() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!location || !city) {
      toast.error("Please select both a map location and a city.");
      return;
    }
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto p-4 sm:p-6">
        <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-gray-800">Add New Room</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block font-medium text-gray-700 mb-2 flex items-center gap-1">
              <MapPin size={18} /> Click on map to set location
            </label>
            <MapContainer
              center={[27.7172, 85.324]}
              zoom={13}
              scrollWheelZoom
              className="h-60 rounded-lg border shadow"
            >
              <TileLayer
              ...
              <LocationSelector
                cities={cities}
                onSelectLocation={({ geo, city: detectedCity }) => {
                  setLocation({ geo });
